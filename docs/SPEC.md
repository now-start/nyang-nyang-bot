...
- 원장 출처는 `ADMIN_ADJUSTMENT`, `PRESENCE_REWARD`, `GOOGLE_SHEET_SYNC`, `REWARD_MANUAL`, `REWARD_ROULETTE`, `CORRECTION`이다.
- Google Sheets는 현재 포인트 잔액의 기준 원본이다. 동기화는 시트에 있는 사용자별 목표 잔액과 원장 `SUM(delta)`의 차액만 `GOOGLE_SHEET_SYNC`로 추가하며, 이미 같으면 원장을 추가하지 않는다.
...
- 영속 엔티티와 application 결과의 변환은 adapter/out에서 명시적으로 수행한다.
...
- Google Sheets 동기화는 관리자 액션이며 시트에 존재하는 사용자 잔액을 원장의 기준값으로 보정한다.
- JVM, JDBC, DB session, Jackson과 컨테이너의 시스템 시간대는 모두 `Asia/Seoul`을 사용한다. 외부 API와 도메인에서는 `Instant`를 유지한다. 레거시 `DATETIME`은 서울 wall-clock으로 해석하고, canonical 절대시각은 `TIMESTAMP(6)`에 UTC로 내부 저장하되 `+09:00` DB session을 통해 읽고 쓴다. 달력 의미가 있는 값만 `DATE`로 저장한다.
