...
- `{streak.current}`와 `{streak.longest}`는 `USER_CALENDAR_DAY` 실행의 non-NULL `calendar_day_started_at`을 `Asia/Seoul` `LocalDate`로 변환해 계산한다. `USER_INTERVAL` 실행에는 달력일 시작 시각이 없으므로 연속 실행일에 포함하지 않는다.
