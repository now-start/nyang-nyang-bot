...
| Phase 5 | 출석/채팅 명령 | 방송 중 운영 흐름 안정화 | 출석 사이클, 관리자 명령어 관리, `!호감도`, `!룰렛결과`, 쿨타임 |
...
- `!룰렛결과` 본인 한정 응답을 추가한다.
- `chat_command` 단일 테이블 기반 관리자 명령어 관리를 구현한다.
- 텍스트 명령어 생성/수정/비활성화, 충돌 검증, 템플릿 미리보기를 제공한다.
...
- `TEXT`, `TRIGGER` 트리거 중복은 DB unique index로 차단된다.
- 관리자 명령어 변경 API는 CSRF 또는 동일 출처/관리자 API 토큰 정책을 통과해야 한다.
