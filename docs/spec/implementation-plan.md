...
- `!룰렛결과` 본인 한정 응답을 추가한다.
- `chat_command` 단일 테이블 기반 관리자 명령어 관리를 추가한다.
- `!호감도`, `!룰렛결과`, 룰렛 후원 명령어는 `TRIGGER` row로 seed한다.
- `ROULETTE_DONATION`은 별도 설정값 없이 현재 활성 룰렛 테이블 1개를 사용한다.
- `TEXT` 명령어 생성/수정/비활성화, 충돌 검증, 템플릿 미리보기를 제공한다.
...
- `TEXT`, `TRIGGER` 트리거 중복은 `chat_command.trigger` unique index로 차단된다.
- 룰렛 후원 명령어는 `ROULETTE_DONATION` action으로 현재 활성 룰렛 테이블을 실행한다.
...
- 룰렛 활성화 검증을 구현한다.
  - 다른 활성 룰렛 테이블이 있으면 활성화 불가
  - 확률 합계 100%
  - `꽝` 필수
  - `꽝` 0% 금지
  - 명령어와 1회 금액 필수
- CHZZK 후원 이벤트 구독과 저장 흐름을 정리한다.
- 후원 메시지에서 `chat_command`의 `ROULETTE_DONATION` 트리거를 토큰 단위 정확 일치로 매칭한다.
- 룰렛 실행은 현재 활성 룰렛 테이블 1개를 기준으로 한다.
...
다음 항목은 1차 기본 동작을 고정하고, 운영 데이터나 명시적 수요가 생기면 재검토한다.
