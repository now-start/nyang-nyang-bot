...
현재 웹 템플릿의 Bootstrap 4.0.0 CDN, Popper, 큰 전역 JS, 커스텀 탭/모달/목록 구현은 기능을 유지한 채 전환할 구현 방식이다.
...
`favorite-list.css` 같은 화면 전용 CSS는 Bootstrap 5 클래스와 Thymeleaf fragment로 대체하고 제거한다.
...
`favorite-list.js`의 역할은 서버 fragment endpoint와 htmx attribute로 이전한다.

전환 규칙:

- JSON 응답을 받아 HTML 문자열을 조립하지 않는다.
- 클릭/탭/필터/저장/검증/미리보기 동작은 `hx-*` attribute로 표현한다.
- 서버는 page endpoint와 fragment endpoint를 분리한다.
...
- htmx runtime.
- OBS 오버레이 전용 JavaScript.
- 브라우저 API가 핵심인 독립 런타임.
