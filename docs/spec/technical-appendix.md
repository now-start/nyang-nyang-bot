...
PRD에 명시되었으나 운영 데이터를 확보한 뒤 결정할 정책은 별도로 묶어 추적한다. 구현 단계에서는 영향 받는 코드를 정책 데이터(관리자 화면)로 빼두고, 정책이 확정되기 전이라도 제품이 1차적으로 동작할 수 있도록 기본 동작을 정의한다.

| 항목 | 미정 사유 | 1차 기본 동작 | 결정 시점 |
| --- | --- | --- | --- |
| 등업 단계명/필요 호감도 (100/1000/5000 단계 명칭 및 혜택) | 운영 사례·시청자 반응이 더 필요 | 등업 기능 비활성. 마이페이지에 등업 표시 없음. | 출시 후 1~2개월 운영 데이터 확보 시 |
| 주간 캐쉬백 (3위까지 10%) 기준 금액·주간 집계 기준·지급 방식 | 실 결제 흐름과 합산 검증 필요 | 주간 채팅 랭킹은 표시하되 자동 캐쉬백 지급 안 함. 필요 시 관리자 수동 지급. | Phase 2~3에서 실 데이터 기반 정의 |
| 룰렛 결과 통계 자동 편향 알람 | 활성화 전 시뮬레이션만으로 운영 시작, 필요성 재평가 | 활성화 전 가상 1만 회 시뮬레이션 미리보기만 노출. 실 운영 통계 알람은 비활성. | 운영 6개월 후 재검토 |
| 외부 인증 업보(5천보 인증 등)의 인증 사진 업로드 시스템 | 관리자 수동 확인으로 충분한지 운영 후 판단 | 인증 업로드 페이지 없이 카페·디스코드 외부 인증을 관리자가 확인하고 수동 적용. | 수요 발생 시 |
| 사용자 탈퇴/개인정보 처리 정책 | CHZZK 미통보로 자동 처리 불가, 별도 정책 수요 발생 시 | 자동 탈퇴 처리 없음. 데이터 보존만. 닉네임 자동 마스킹 안 함. | 법적 요구나 운영 수요 발생 시 |
