...
- 설정 카드: 5열 그리드 — 이름 / 후원 키워드(`!룰렛`, green 텍스트) / 1회 금액 / 최대 순차 애니메이션 / 공개·활성 토글 2개(36×20 pill 토글)
...
- 편집 폼(카드): 호출어+쿨타임 → 메시지 템플릿(변수 카탈로그에서 `{viewer.nickname}`, `{favorite.balance}` 등을 선택) → **중복 검증 결과 배너** → 미리보기(`냥냥봇:` + `치즈냥` 샘플) → [비활성화](danger outline, 좌) [취소][저장](우)
