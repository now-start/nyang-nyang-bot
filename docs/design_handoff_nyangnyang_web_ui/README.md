...
`docs/SPEC.md`와 현재 Thymeleaf 화면 계약을 기준으로 하며, 사용자가 선택한 **1c 소프트 엘리베이션** 비주얼 톤(그린 틴트 다크 그레이 + 섀도 카드 + CHZZK Neon Green 단일 액센트)으로 전체 화면을 전개했다.
...
### 1. 호감도 보드 (`/points/list`) — 디자인 id `1c`
- 헤더(그라데이션 배경): 좌측 로고 사각형(44px, green bg, radius 14) + "치즈냥 호감도" 20px/700 + 채널·권한 서브텍스트. 우측 [데이터 동기화](secondary) [생존자 보상](primary, glow)
...
- 상태: 빈 목록, 검색 결과 없음, 히스토리 로딩(skeleton), 권한 없음(toast)을 구분
...
- 스탯 3열: 현재 호감도 875(green 26px) / 누적 리워드 포인트 반영 +42 / 최근 룰렛 결과 호감도 +10
- 보유 리워드(쿠폰·미션 등): 필터 pill 4개(OWNED 기본 선택=green fill, 나머지 outline). 행 그리드 `96px 1fr 110px 200px` (일시/이름+NEW/상태 배지/환산 정보). 원장 참조 번호는 표시 전용 텍스트로 제공
- 전체 히스토리: 단일 카드 테이블. 그리드 `110px 90px 70px 90px 1fr` (일시/분류 pill/증감/반영 후/설명). 정정 행은 red pill로 구분하며 원본 거래 이동 링크는 제공하지 않음. "최신순 · 50건 단위" 표기

### 4. 생존자 보상 모달 (`/presence-rewards/*`) — `2c`
- 모달 560px, 구조: header(제목 + "수집 중" 상태 pill + ✕) → 설정(지급 포인트 input, 값은 `+1`처럼 부호 표시, green 텍스트 / 대상자 24명·선택 22명 + [새로고침]) → 대상자 체크리스트(max-height 230, 내부 스크롤, 최근 채팅 시각 표시) → footer("사이클·사용자당 1회만 지급" 안내 + [취소하고 종료](danger outline) [선택 22명에게 +1 적용](primary))
- 적용 버튼 라벨에 인원수·지급값을 동적으로 포함

### 5. 룰렛 설정 관리 (`/admin/roulette/configs`) — `2d`
- 헤더 + [1만 회 시뮬레이션](secondary) [새 룰렛](primary)
- 설정 카드: 이름 / 후원 키워드(`!룰렛`, green 텍스트) / 1회 금액 / 고회차 경고 기준 / 상태(`DRAFT`, `ACTIVE`, `ARCHIVED`). 활성 설정은 한 개만 허용
- 옵션 테이블: 그리드 `1fr 120px 170px 120px` (옵션/확률 % 4소수 우측정렬/리워드 타입 enum/액션). "꽝" 행은 "필수" 칩 + 액션 "고정"
- **검증 배너(핵심)**: green tint 배너에 "확률 합계 100.0000%" + "꽝 포함 OK" + 옵션 수 + [활성화]. 합계≠100이면 red 배너로 전환하고 활성화 버튼 disabled
- 시뮬레이션 카드: 기대 확률/시뮬레이션/차이(+green, -red)
- 최근 실행 목록은 `/admin/roulette/runs`, 재송출은 `/admin/overlay/roulette/runs/{rouletteRunId}/replay`를 사용

### 6. 명령어 관리 (`/admin/commands`) — `2e`
- 2-column: 좌측 목록(1.15fr), 우측 편집 폼(1fr)
- 목록 행: 그리드 `110px 1fr 64px` — 호출어(green/700) / 응답 템플릿 요약(ellipsis) / 상태 배지(활성=green tint, 비활성=neutral + 행 opacity 0.65)
- 편집 중 행: green 보더 + "마지막 수정" 메타
- 편집 폼(카드): 호출어+쿨타임 → 메시지 템플릿(변수 카탈로그에서 `{viewer.nickname}`, `{point.balance}` 등을 선택) → **중복 검증 결과 배너** → 미리보기(`냥냥봇:` + `치즈냥` 샘플) → [비활성화](danger outline, 좌) [취소][저장](우)
- 모든 채팅 명령어는 같은 템플릿 실행 경로를 사용하며 호출어·템플릿·활성 상태·쿨타임을 수정할 수 있다.

### 7. 관리자 포인트/리워드 처리 모달 — `3a`
- 모달 720px. 구조: header("관리자 처리" + "ADMIN 전용" 칩 + ✕) → 대상 검색 + 선택 사용자 카드(아바타 이니셜, 닉네임, channelId 축약, 현재 호감도 green) → 처리 유형 pill 5개(포인트 지급/차감 · 리워드 지급 · 쿠폰 구매 · 사용 처리 · 정정) → 프리셋 select + 증감값(`+10` 부호 필수, green, 우측정렬) → 공개 설명 / 관리자 메모(비공개 칩 + **dashed 보더**로 구분) → footer 미리보기 "현재 875 + +10 = 적용 후 885"(적용 후는 green tint 박스) + [취소][적용]
...
- 재송출 카드: "재송출은 원장에 재반영되지 않습니다" 캡션 필수. 실행 행: 일시/요약/상태(표시 완료=green, 표시 실패=red)/[재송출] — 실패 건은 primary fill로 강조
...
- SUMMARY 패널: "추가 6회 결과" + 옵션×개수 리스트(당첨=green, 꽝=gray)
- 타이밍: 진입 300ms → 회전 1800ms → 감속 900ms → 결과 강조 1500ms → 퇴장 300ms. 최종 정지 결과는 서버 확정값과 일치
...
- htmx fragment 교체 대상: 보드 목록+페이지네이션, 히스토리 펼침 영역, 생존자 목록, 룰렛 검증 배너, 최근 실행 목록, 명령어 편집 폼+미리보기, 토큰 영역
- 생존자 보상: 사이클 상태(수집 중/종료), 선택 인원 수를 footer 버튼 라벨에 반영
...
- 오버레이 휠은 CSS conic-gradient — 실제 구현 시 옵션 수에 따라 동적 세그먼트

## Files
- `냥냥봇 디자인.dc.html` — 전체 디자인 캔버스 (턴3 → 턴1 순, 최신이 위). 브라우저에서 직접 열어 확인
- 제품 계약: 코드베이스의 `docs/SPEC.md`; 실제 경로와 fragment는 현재 Controller·Thymeleaf 템플릿을 기준으로 한다.
