...
실행 가능한 Flyway `V6.1`~`V10`과 Java migration을 구현했으며, 최종 물리 스키마 명세는
[`target-schema.sql`](./target-schema.sql), MariaDB 불변성 명세는
[`canonical-invariants.sql`](../../src/main/resources/db/canonical-invariants.sql), 현행 DB 사전점검 쿼리는
...
`command_execution`은 현행 runtime에 없는 신규 요구이며 빈 상태로 시작한다. 생존자
보상은 관리자가 임의 시점의 채팅 생존자를 골라 포인트를 주는 기능이다.
...
- `USER_INTERVAL`: 마지막 승인 시각부터 `user_cooldown_seconds`가 지난 뒤 다시 승인한다.
  `calendar_day_started_at`은 NULL이고 같은 사용자·명령의 이벤트를 여러 건 저장한다.
- `USER_CALENDAR_DAY`: `user_cooldown_seconds`는 NULL이고 승인 시점의
  `Asia/Seoul` 달력일 00:00을 나타내는 `Instant`를 저장한다.
  `UNIQUE(command_id, user_id, calendar_day_started_at)`이 같은 날의 두 번째 승인을
  차단한다.
...
`executed_at`과 `calendar_day_started_at`은 DB가 반환한 동일한 승인 `Instant`에서 계산한다.
`executed_at`은 `TIMESTAMP(6)`에 UTC instant로 내부 저장하고 `+09:00` DB session에서
서울 wall-clock으로 읽는다. `calendar_day_started_at`은 같은 instant가 속한 서울 달력일의
00:00에 해당하는 절대시각이며 upstream 채팅 시각은 사용하지 않는다.

- 전체 count: 현재 이벤트 insert 뒤 `command_id`로 `COUNT(*)`
- 사용자 count: 현재 이벤트 insert 뒤 `command_id`, `user_id`로 `COUNT(*)`
- 일일 실행일: 같은 command/user의 non-NULL `calendar_day_started_at`을 서울
  `LocalDate`로 변환한 날짜 집합
- longest streak: 변환한 일일 실행일 중 가장 긴 연속 날짜 구간
...
erDiagram
    ...
    COMMAND_EXECUTION {
        ...
        varchar user_id FK
        timestamp calendar_day_started_at UK "TIMESTAMP(6); nullable; with command/user"
    }
    TIMER_MESSAGE {
        bigint id PK
        varchar created_by_user_id FK
    ...
    WEEKLY_CHAT_COUNT {
        bigint id PK
        varchar user_id FK
        timestamp week_started_at UK "TIMESTAMP(6); with user"
    }
    DONATION {
        bigint id PK
...
`(command_id, user_id, calendar_day_started_at)`, `(week_started_at, user_id)`는 업무
`UNIQUE`로 보존한다. MariaDB UNIQUE가 여러 NULL을 허용하므로 interval 실행의
`calendar_day_started_at=NULL` 행은 계속 쌓이고, 일일 정책의 non-NULL 서울 달력일 시작
시각만 하루 한 건으로 제한된다.
...
| `command_execution`의 정책 snapshot·`calendar_day_started_at` | 승인 당시 정책 설명과 일일 중복 방지·streak 계산 |
...
- 명령 실행은 현재 설정과 별도로 승인 당시 정책과 interval seconds를 snapshot으로
  보존한다. `USER_INTERVAL`은 seconds가 5~3600이고 `calendar_day_started_at`이
  NULL이어야 하며, `USER_CALENDAR_DAY`는 seconds가 NULL이고 서울 달력일 시작 절대시각이
  있어야 한다.
- `UNIQUE(command_id, user_id, calendar_day_started_at)`은 일일 정책의 사용자별 하루
  한 번을 보장한다. interval 행의 NULL 시작 시각은 중복으로 취급하지 않는다.
  전체/사용자별 count는 DB `COUNT(*)`, streak는 non-NULL 시작 시각을 서울
  `LocalDate`로 변환한 집합으로 계산한다. MariaDB INSERT trigger는 Unix epoch 연산으로
  `executed_at`이 속한 서울 달력일 시작 절대시각과 `calendar_day_started_at`이 같은지
  session time zone과 무관하게 검증한다.
...
- 주간 집계의 `week_started_at`은 Unix epoch 기준 CHECK로 서울 월요일 00:00 절대시각만
  허용하며 session time zone에 의존하지 않는다.
...
| `favorite_history(SHEET_MIGRATION)` | `point_ledger_entry(GOOGLE_SHEET_SYNC)` | 기존 시트 기준 잔액 보정 이력의 의미를 명확한 canonical source로 변경 |
| `favorite_adjustment` | `point_adjustment_preset` | 금액을 BIGINT로 확대 |
| `weekly_chat_rank` | `weekly_chat_count` | 닉네임·공통 시각 제거, legacy `week_start_date`를 서울 월요일 00:00 `week_started_at`으로 변환 |
...
`UPBO_ROULETTE -> REWARD_ROULETTE`, `ATTENDANCE -> PRESENCE_REWARD`,
`SHEET_MIGRATION -> GOOGLE_SHEET_SYNC`로 source enum을 바꾸고 다른 source는 유지한다.
여기서 legacy `ATTENDANCE`는 관리자가 채팅 생존자를 선택해 지급한 포인트 원인을
뜻하며 사용자가 실행하는 일별 명령과 관계없다.
...
레거시 시각 컬럼은 timezone 정보가 없는 `DATETIME`이고 기존 runtime은 JVM 기본 zone의
`LocalDateTime`을 저장했다. 운영 DB의 `SYSTEM/KST`와 세션 시각 차이는 기존 값이
`Asia/Seoul` wall-clock이라는 근거다. V8은 `+09:00` session에서 레거시
`LocalDateTime`을 canonical `TIMESTAMP(6)`에 기록하므로 수동 `+9`/`-9` 보정을 하지
않아도 동일한 서울 시각에 해당하는 UTC instant로 저장된다. target runtime의 도메인 값은
`Instant`이고, MariaDB는 `TIMESTAMP(6)`을 UTC로 내부 저장한 뒤 session time zone에 맞춰
변환한다. canonical 달력 경계인 `command_execution.calendar_day_started_at`과
`weekly_chat_count.week_started_at`도 각각 서울 달력일 00:00과 월요일 00:00에 해당하는
`Instant`를 `TIMESTAMP(6)`에 저장한다. 레거시 `weekly_chat_rank.week_start_date DATE`는
서울 월요일 00:00 절대시각으로 변환한다. 백필은 과거 JVM·DB 설정 증거를 보존하고
`nyang.migration.legacy-datetime-zone=Asia/Seoul` 또는
`NYANG_LEGACY_DATETIME_ZONE=Asia/Seoul`을 명시한 경우에만 시작한다. 확인 결과가 다른
zone이라면 source zone과 적용 기간을 확정한 별도 정규화 migration을 먼저 설계한다.

MariaDB 10.11의 `TIMESTAMP` 범위 때문에 서울 wall-clock으로 해석한 모든 백필 후보는
`[1970-01-01 09:00:01, 2038-01-19 12:14:08)` 안에 있어야 한다. OAuth의
`modify_date + expires_in`과 비활성 overlay token의
`COALESCE(revoked_at, modify_date, create_date)`처럼 계산으로 생성되는 target 시각도 같은
검사를 받는다. 장기 운영을 위해 2038년보다 충분히 앞서 `TIMESTAMP` 상한이 확장된
MariaDB 11.5 이상으로 업그레이드한다.
...
1. `V6_1__validate_canonical_timestamp_session.java`
   - MariaDB session의 유효 offset이 `+09:00`이 아니거나
     `explicit_defaults_for_timestamp=ON`이 아니거나 SQL mode에 `MAXDB`가 있으면
     shadow DDL을 하나도 실행하지 않고 중단
2. `V7__create_canonical_shadow_schema.sql`
   - `next_*` 이름으로 목표 16개 테이블, FK, 인덱스, CHECK, COMMENT를
     `IF NOT EXISTS`로 생성
3. `V7_1__install_canonical_shadow_invariants.java`
   - 기존 shadow trigger를 제거한 뒤 `db/canonical-invariants.sql`의 24개 trigger를
     `next_*` 테이블과 `trg_next_*` 이름으로 다시 설치
4. `V8__backfill_canonical_data.java`
   - MariaDB에서는 `nyang.migration.canonical-backfill-approved=true` 시스템 속성 또는
     `NYANG_CANONICAL_BACKFILL_APPROVED=true` 환경 변수가 없으면 OAuth 평문 token을
     shadow 테이블에 복제하기 전에 중단
   - 레거시 JVM/DB의 `DATETIME` zone을 `Asia/Seoul`로 명시하지 않으면 백필 전에 중단
   - DB session의 유효 offset이 `+09:00`이 아니거나
     `explicit_defaults_for_timestamp=ON`이 아니거나 SQL mode에 `MAXDB`가 있으면 중단
   - 레거시 시각과 OAuth/overlay 계산 시각이 MariaDB 10.11 `TIMESTAMP` 범위를 벗어나면
     target insert 전에 중단
...
5. `V8_1__materialize_roulette_history.java`
...
6. `V8_2__backfill_roulette_dependents.java`
...
7. `V9__validate_canonical_backfill.java`
   - MariaDB transaction isolation이 `REPEATABLE READ` 또는 `SERIALIZABLE`이 아니면
...
8. `V10__cutover_canonical_schema.java`
...
9. 새 JPA 모델로 시작하고 Hibernate `ddl-auto=validate` 수행

실행도 두 단계로 분리한다. 첫 번째 Flyway 실행은 target을 `9`로 고정하고 backfill 승인과
레거시 zone만 제공해 shadow 생성·백필·검증까지 완료한다. 결과를 확인한 뒤 두 번째 실행에서
target `10`과 cutover 승인만 제공한다. V8 승인으로 V10까지 연속 진행하지 않는다.
...
않는다. V8의 레거시 zone 명시도 preflight에 기록한 기존 JVM/DB 시간대 증거를 대신하지
...
않는다. V6.1이 V7 raw DDL보다 먼저 session 계약을 검증하므로 시간 설정 위반은 shadow
테이블을 남기지 않는다. V7, V7.1, V8.1과 V10의 부분 실행 복구를 MariaDB 10.11에서 실제로 검증한다.
...
name 순서로 V6.1~V10 Java migration checksum에 포함된다. 이 migration들이
...
- `USER_INTERVAL`은 같은 트랜잭션에서 최신 `(command_id, user_id)` 이벤트도
  ...
  직렬화된다.
- 승인 트랜잭션은 DB의 `CURRENT_TIMESTAMP(6)`를 한 번 읽어 공통 승인 시각으로 사용한다.
  JDBC/Hibernate와 도메인은 이를 동일한 `Instant`로 다루고 DB `TIMESTAMP(6)`은 UTC로
  내부 저장한다. `calendar_day_started_at`은 같은 instant가 속한 서울 달력일의 00:00
  절대시각으로 계산한다. `LocalDate`는 이 서울 달력 규칙을 계산할 때만 일시적으로
  사용한다. 서로 다른 애플리케이션 인스턴스의 로컬 시계나 upstream 채팅 시각은
  쿨다운·날짜 판정에 사용하지 않는다. Config Server는
  `hibernate.type.preferred_instant_jdbc_type=TIMESTAMP`만 명시해 Hibernate의 기본
  `TIMESTAMP_UTC` 바인딩이 MariaDB session에서 절대시각을 9시간 이동시키지 않게 한다.
  JVM과 MariaDB의 system/session time zone은 배포 환경에서 `Asia/Seoul`로 통일하며,
  애플리케이션에 Hibernate/Jackson time zone이나 Hikari connection init SQL을 중복
  지정하지 않는다. MariaDB 전역 `explicit_defaults_for_timestamp=ON`을 유지하고 SQL
  mode의 `MAXDB`는 `TIMESTAMP`를 `DATETIME`으로 변환하므로 허용하지 않는다.
...
- 전체/사용자별 count는 repository `COUNT` projection으로 계산하고, 일일 명령의
  current/longest streak만 해당 사용자의 non-NULL `calendar_day_started_at`을 읽어 서울
  `LocalDate`로 변환한 뒤 애플리케이션에서 계산한다.
...
- canonical 절대시각 컬럼 35개는 모두 `TIMESTAMP(6)`이고 `DATETIME`과 `DATE`는 0개다.
  서울 달력 경계는 `command_execution.calendar_day_started_at`과
  `weekly_chat_count.week_started_at`에 절대시각으로 저장하며 DB session의 유효 offset은
  항상 `+09:00`이다.
...
- interval 실행 다건 허용, 일일 명령의 사용자·날짜 UNIQUE, count·streak를 MariaDB
  ...
  테스트한다.
- 같은 command의 동시 승인이 서로 다른 연속 count를 렌더링하는지,
  `executed_at`이 속한 서울 달력일 시작과 일치하지 않는 `calendar_day_started_at`이 DB
  session time zone과 무관하게 INSERT trigger에서 거부되는지 검증한다.
...
- command 초기화와 승인을 경합시켜 초기화 commit 뒤 이벤트가 다시 생기지 않는지,
  서울 자정 양쪽의 DB 승인 시각이 서로 다른 `calendar_day_started_at`으로 저장되는지
  검증한다.
