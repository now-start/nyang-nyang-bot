...
`executed_at`과 `calendar_date`는 DB가 반환한 동일한 승인 `Instant`에서 계산한다.
`executed_at`은 `TIMESTAMP(6)`에 UTC instant로 내부 저장하고 `+09:00` DB session에서
서울 wall-clock으로 읽는다. `calendar_date`도 같은 instant의 서울 날짜를 사용하며 upstream
채팅 시각은 사용하지 않는다.
...
- `UNIQUE(command_id, user_id, calendar_date)`는 일일 정책의 사용자별 하루 한 번을
  ...
  `+09:00` session에서 보이는 `executed_at`의 서울 날짜가 일일 이벤트의
  `calendar_date`와 같은지도 검증한다.
...
레거시 컬럼은 timezone 정보가 없는 `DATETIME`이고 기존 runtime은 JVM 기본 zone의
...
변환한다. 달력값은 `command_execution.calendar_date`와
`weekly_chat_count.week_start_date` 두 `DATE` 컬럼에만 저장한다. 백필은 과거 JVM·DB
설정 증거를 보존하고
...
- 승인 트랜잭션은 DB의 `CURRENT_TIMESTAMP(6)`를 한 번 읽어 공통 승인 시각으로 사용한다.
  ...
  내부 저장한다. `calendar_date`는 같은 instant의 서울 날짜로 계산한다. 서로 다른
  애플리케이션 인스턴스의 로컬 시계나 upstream 채팅 시각은 쿨다운·날짜 판정에 사용하지
  않는다. Hibernate의 `hibernate.type.preferred_instant_jdbc_type=TIMESTAMP`와
  `hibernate.jdbc.time_zone=Asia/Seoul`을 함께 고정해 `Instant` 바인딩도 DB session과
  같은 기준을 사용한다. non-local MariaDB Hikari connection은 생성 시 `time_zone='+09:00'`과
  `explicit_defaults_for_timestamp=ON`을 설정해 조회값, column default와 trigger의 DB
  시각이 같은 계약을 따르게 한다. SQL mode의 `MAXDB`는 `TIMESTAMP`를 `DATETIME`으로
  변환하므로 허용하지 않는다.
...
- canonical 절대시각 컬럼 33개는 모두 `TIMESTAMP(6)`이고 `DATETIME`은 0개다. 달력값은
  `command_execution.calendar_date`, `weekly_chat_count.week_start_date` 두 `DATE`만
  존재하며 DB session의 유효 offset은 항상 `+09:00`이다.
...
- interval 실행 다건 허용, 일일 명령의 사용자·날짜 UNIQUE, count·streak를 MariaDB
  ...
  테스트한다.
- 같은 command의 동시 승인이 서로 다른 연속 count를 렌더링하는지, `+09:00` session에서
  보이는 `executed_at`의 서울 날짜와 일치하지 않는 `calendar_date`가 INSERT trigger에서
  거부되는지 검증한다.
