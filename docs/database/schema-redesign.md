...
1. `V7__create_canonical_shadow_schema.sql`
   - `next_*` 이름으로 목표 16개 테이블, FK, 인덱스, CHECK, COMMENT 생성
...
2. `V8__backfill_canonical_data.sql`
   - 사용자 union, OAuth, command, timer, point, weekly, donation,
     overlay access token의 독립적인 SQL 백필
...
- 현행 관리자 출석 수집 기능은 일일 명령과 구분되도록 내부 `attendance` 패키지와
  `AttendanceService`를 presence check 도메인으로 변경하고, 선택된 생존자 포인트 원인의
